...
document.addEventListener("DOMContentLoaded", () => {
    ...
    getInspoBttn.addEventListener("click", () => {
        ...
        function handleCreateInspo() {
            ...
            let newInspoObj = {
                author: createAuthorNameInput[0].value,
                quote: createInspoQuote[0].value,
                image: createInspoImg[0].value,
            }

            // argument is a single list
            fetchObject(newInspoObj);
            createAuthorNameInput[0].value = "";
            createInspoQuote[0].value = "";
            createInspoImg[0].value = "";
    
            // inspoCollection.append(saveForm)
            console.log(newInspoObj);
    
        };

    })   

    function updateObject(inspo) {
        fetch((`http://localhost:3000/inspo/${inspo.id}`), {
            method: 'PATCH',
            headers:
            {
            ...
            body: JSON.stringify(inspo),
            // reloadFetch();
            })
        }

        // Arguments: 
        // newInspoObj -> List of dictionaries corresponding to quote blocks
        // const2 -> Integer representing age
        function fetchObject(newInspoObj) {
        fetch(('http://localhost:3000/inspo'), {
            method: 'POST',
            headers:
            {
                "Content-Type": "application/json",
            ...
            body: JSON.stringify(newInspoObj),
            // reloadFetch();
            })
            dropDownMenuFunc([newInspoObj]);
        }
